...
theorem step_spec (a b c : Nat) (s : RemainderStep Rat) :
    ...
    ((RemainderStep.clearDenominators a b c s).leftScale : Rat) * a = k * s.leftScale ∧
      HexPolyZMathlib.toPolyℚ (RemainderStep.clearDenominators a b c s).quotient * C (b : Rat) =
        C k * toPolynomial s.quotient ∧
      ((RemainderStep.clearDenominators a b c s).rightScale : Rat) * c = k * s.rightScale := by
  dsimp only
  constructor
  · simp only [RemainderStep.clearDenominators, Int.cast_mul, Int.cast_natCast, Nat.cast_mul, cast_num]
    ring
  constructor
  · rw [RemainderStep.clearDenominators, ← HexPolyZMathlib.toPolynomial_toRatPoly, ZPoly.toRatPoly_scale_int,
...
private theorem chain_entry (p : DensePoly Rat) (g : Nat) (cert : SignedRemainderChain Rat) (i : Nat) :
  ...
  · have hi' : i < (SignedRemainderChain.clearDenominators p g cert).chain.size := by
      simpa only [SignedRemainderChain.clearDenominators, Hex.Array.size_map'] using hi
    rw [← Array.getElem_eq_getD (h := hi') 0, ← Array.getElem_eq_getD (h := hi) 0]
    simp only [SignedRemainderChain.clearDenominators, Hex.Array.getElem_map']
  · have hi' : ¬ i < (SignedRemainderChain.clearDenominators p g cert).chain.size := by
      simpa only [SignedRemainderChain.clearDenominators, Hex.Array.size_map'] using hi
...
private theorem step_terms (a b c : Nat) (s : RemainderStep Rat)
    ...
    let k : Rat := (a * b * c * s.leftScale.den * s.rightScale.den *
      (ZPoly.clearDenominators s.quotient).1 : Nat)
    C ((RemainderStep.clearDenominators a b c s).leftScale : Rat) * p' = C k * (C s.leftScale * p) ∧
      HexPolyZMathlib.toPolyℚ (RemainderStep.clearDenominators a b c s).quotient * q' = C k * (toPolynomial s.quotient * q) ∧
      C ((RemainderStep.clearDenominators a b c s).rightScale : Rat) * r' = C k * (C s.rightScale * r) := by
...
private theorem step_add (a b c : Nat) (s : RemainderStep Rat)
    ...
    C ((RemainderStep.clearDenominators a b c s).leftScale : Rat) * p' =
      HexPolyZMathlib.toPolyℚ (RemainderStep.clearDenominators a b c s).quotient * q' +
        C ((RemainderStep.clearDenominators a b c s).rightScale : Rat) * r' := by
...
private theorem step_checks (p q r : DensePoly Rat) (s : RemainderStep Rat)
    ...
    SignedRemainderChain.checkStep Int.sign (ZPoly.clearDenominators p).2
      ...
      (RemainderStep.clearDenominators (ZPoly.clearDenominators p).1 (ZPoly.clearDenominators q).1
...
private theorem initial_checks (p g : DensePoly Rat) (g' : ZPoly) (d : Nat) (hd : 0 < d)
    ...
    Int.sign (SignedRemainderChain.clearDenominators p d cert).initial.leftScale = 1 ∧
      Int.sign (SignedRemainderChain.clearDenominators p d cert).initial.rightScale = 1 ∧
      SignedRemainderChain.subIsZero
        (DensePoly.scale (SignedRemainderChain.clearDenominators p d cert).initial.leftScale
          (g' * (ZPoly.clearDenominators p).2.derivative))
        ((SignedRemainderChain.clearDenominators p d cert).initial.quotient * (ZPoly.clearDenominators p).2 +
          DensePoly.scale (SignedRemainderChain.clearDenominators p d cert).initial.rightScale ((SignedRemainderChain.clearDenominators p d cert).chain.getD 1 0)) = true := by
  ...
  simpa only [SignedRemainderChain.clearDenominators, HexPolyZMathlib.toPolyℚ, toPolynomial_scale, toPolynomial_mul,
...
theorem chain_checks (p g : DensePoly Rat) (g' : ZPoly) (d : Nat) (hd : 0 < d)
    ...
    SignedRemainderChain.check Int.sign (ZPoly.clearDenominators p).2
      g' (SignedRemainderChain.clearDenominators p d cert) = true := by
  have hi := initial_checks p g g' d hd hclear cert h
  have hsize : (SignedRemainderChain.clearDenominators p d cert).chain.size = cert.chain.size := by
    simp only [SignedRemainderChain.clearDenominators, Hex.Array.size_map']
  have hsrc := h
  ...
  · simp only [SignedRemainderChain.clearDenominators, Hex.Array.map'_eq_map, Array.getElem?_map, hh, Option.map_some]
  · rw [← hsize]
    apply Array.all_eq_true_iff_forall_mem.mpr
    intro q hq
    simp only [SignedRemainderChain.clearDenominators, Hex.Array.map'_eq_map, Array.mem_map] at hq
    obtain ⟨r, hr, rfl⟩ := hq
  ...
  · by_cases hs : cert.chain.size = 1
    · simp only [hs, ↓reduceIte, Bool.and_eq_true] at ht ⊢
      constructor
      · simpa only [SignedRemainderChain.clearDenominators, Array.isEmpty, Hex.Array.size_ofFn'] using ht.1
      · simpa only [SignedRemainderChain.clearDenominators, Option.isNone_map] using ht.2
    · simp only [hs, ↓reduceIte, Bool.and_eq_true, decide_eq_true_eq, and_assoc] at ht ⊢
      refine ⟨?_, ?_, ?_⟩
      · simpa only [SignedRemainderChain.clearDenominators, Hex.Array.size_ofFn'] using ht.1
      · apply Array.all_eq_true_iff_forall_mem.mpr
        intro i hi
        have hidx : i < cert.steps.size := by
          simpa only [SignedRemainderChain.clearDenominators, Hex.Array.size_ofFn'] using Array.mem_range.mp hi
        ...
        have hi' : i < (SignedRemainderChain.clearDenominators p d cert).steps.size := by simpa only [SignedRemainderChain.clearDenominators, Hex.Array.size_ofFn'] using hidx
        rw [← Array.getElem_eq_getD (h := hi') ⟨0, 0, 0⟩]
        simpa only [SignedRemainderChain.clearDenominators, Hex.Array.getElem_ofFn', Fin.getElem_fin,
...
private theorem lastIsConstant_eq (p : DensePoly Rat) (d : Nat) (cert : SignedRemainderChain Rat) :
  ...
  simp only [SignedRemainderChain.lastIsConstant, show (SignedRemainderChain.clearDenominators p d cert).chain.size = cert.chain.size by
    simp only [SignedRemainderChain.clearDenominators, Hex.Array.size_map'], chain_entry, clear_size]
...
theorem signs_eq (p : DensePoly Rat) (d : Nat) (cert : SignedRemainderChain Rat) (x : Dyadic) :
    TarskiCertificate.signs Sturm.orderSign (EndpointSigns.ofSign Sturm.orderSign) cert.chain
        (.finite x.toRat) =
      TarskiCertificate.signs Int.sign EndpointSigns.intDyadic (SignedRemainderChain.clearDenominators p d cert).chain (.finite x) := by
  apply signs_rat_eq
  · simp only [SignedRemainderChain.clearDenominators, Hex.Array.size_map']
  · intro i
...
theorem certificate_checks {Ctx : Type u} [DecidableEq Ctx] (context : Ctx)
    ...
    TarskiCertificate.check Int.sign EndpointSigns.intDyadic context
      ...
      (.finite I.lower) (.finite I.upper) value (TarskiCertificate.clearDenominators p g I cert) = true := by
