...
/-! ### Bridge-bound size-reduce updates

The following five `scaledCoeffs_sizeReduce_*` / `gramDet_sizeReduce`
theorems have Hex-local statements but proofs that ultimately reach
`Matrix.bareiss_eq_det` through `gramDet_rowAdd_earlier`. Per
[SPEC/Libraries/hex-gram-schmidt.md "Proof path governs placement, not
just statement"] they belong in `HexGramSchmidtMathlib/Update.lean`,
and HO-43 Round 2 (#3899) targets their relocation. They remain here
pending the consumer-side refactor of `LLLState.sizeReduceColumn` in
`HexLLL/Basic.lean` (#3916): that constructor discharges its `ν_eq`
and `d_eq` fields by calling these theorems, so moving them out
without the consumer refactor would either cascade `sorry` into the
LLL state invariants or smuggle a bridge import into the Mathlib-free
`HexLLL.Basic`. Once #3916 lands, this block migrates to the bridge.
-/
