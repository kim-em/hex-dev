...
performance demonstration: benchmarked against `strassenDefault` on the prime
field it is roughly `1.4`–`3.2×` faster across `64…256`, both as shipped (each
config at its own measured cutoff) and at matched cutoffs (the controls in the
committed JSON, isolating the base-kernel effect from the cutoff tuning). The
executable path is engineered for this: the scalar tail-recursive loops
`delayedDotLoop`/`delayedDotRun` keep the two-word accumulator in machine words
...
`m < barrettWindow` fast path), `barrettBaseMul` transposes `N` once (mirroring
`mulImpl`), and `radixResidue` computes `2^64 % p` in machine words. Each piece
is verified against the unchanged `foldReduce` reference. The committed
