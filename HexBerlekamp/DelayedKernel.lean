...
`65537`, and `2^31 - 1`, its leaf kernel is `1.25`–`1.44×` faster and full
Strassen is `1.22`–`1.42×` faster across the committed square and rectangular
cases. The matched-cutoff controls are also positive. Inner dimensions `768`
and `1280` cross the 256-term reduction window three and five times, so the
measurement covers the periodic path rather than only the final flush. The
