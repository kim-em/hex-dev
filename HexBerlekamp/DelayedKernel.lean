...
performance demonstration: benchmarked against default-base `mulStrassen` on the
prime field it is roughly `1.4`–`3.2×` faster across `64…256`. The original
implementation measured `7`–`10×` *slower*; closing that gap took four
executable-side changes, each verified against the unchanged `foldReduce`
reference: the boxed `(UInt64 × UInt64 × Nat)` accumulator threaded through
`Fin.foldl` became the scalar tail-recursive loops
`delayedDotLoop`/`delayedDotRun` (allocation-free, carry inlined as a wraparound
compare instead of the `UInt64.addCarry` extern, window counter dropped entirely
on the `m < barrettWindow` fast path); `barrettBaseMul` transposes `N` once
instead of materializing a strided column per output entry (mirroring `mulImpl`);
`radixResidue` computes `2^64 % p` in machine words instead of a bignum `Nat`
reduction per dot product; and the config cutoff moved to the delayed kernel's
own measured crossover. The committed comparison lives in
`reports/figures/strassen-base-kernel-comparison.svg`, regenerated by
`lake exe hexstrassen_compare` and
...
fits in a single window (`m < barrettWindow`, i.e. always at base-kernel sizes):
the accumulator never flushes mid-run, so the window counter and its per-term
check disappear from the inner loop entirely. The carry is written as an
...
Mirroring `mulImpl`, the fast path transposes `N` once so every dot product runs
over two contiguous rows, instead of materializing a strided column vector per
output entry. Both branches equal the reference `mul`, so the config built from
this is `Valid`. -/
@[expose]
def barrettBaseMul (ctx : Hex.BarrettCtx p) {n m k : Nat}
    (M : Matrix (ZMod64 p) n m) (N : Matrix (ZMod64 p) m k) : Matrix (ZMod64 p) n k :=
  if 2 ≤ m then
    let Nt := Matrix.transpose N
    Matrix.ofFn fun i j => delayedDot ctx (Matrix.row M i) (Matrix.row Nt j)
  else
    Matrix.mulImpl M N
...
theorem barrettBaseMul_eq_mul (ctx : Hex.BarrettCtx p) {n m k : Nat}
  ...
  · apply Matrix.ext_getElem
    intro i j
    rw [Matrix.getElem_ofFn, Matrix.row_transpose, delayedDot_eq_dotProduct]
...
Measurably faster than `strassenDefault` at every measured dimension (see the
measured outcome in the module docstring and the committed comparison in
`reports/figures/`). Its cutoff is pinned at `128`, the delayed kernel's own
measured crossover: the delayed dot product has a lower per-term cost but a
per-dot-product reduction constant, so it profits from longer base-case dot
products than the default kernel does (the default cutoff is measured separately
and may move). -/
@[expose]
...
into a non-default `StrassenConfig` (with a distinct cutoff), exercising the
pluggable-base-kernel path with a verified-`Valid` config supplied by the caller
independently of any performance claim. The *performance* demonstration required
by honesty constraint (b) of `HexMatrix/SPEC/hex-matrix.md` § "A demonstration
non-default config" is `strassenBarrett` above, which measurably beats the
default; `strassenDemo` originally filled that role as the honest fallback while
the delayed kernel was slower, and stays as the minimal plug-in example. -/
@[expose]
