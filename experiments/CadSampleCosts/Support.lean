...
elab_rules : command
  | `(#cad_emit $name:ident : $goal:term) => liftTermElabM do
    ...
    let height (p : Hex.ZPoly) : Nat := p.toArray.foldl (fun h a => max h a.natAbs) 0
    let (carrierDegree, carrierHeight, chainHeight, chainLength) : Nat × Nat × Nat × Nat := match result.certificate with
      | .cells data => (data.carrier.carrier.natDegree, height data.carrier.carrier,
          data.carrier.replay.chain.foldl (fun h p => max h (height p)) 0,
          data.carrier.replay.chain.size)
      | .noRoots data => (data.carrier.carrier.natDegree, height data.carrier.carrier,
          data.carrier.replay.chain.foldl (fun h p => max h (height p)) 0,
          data.carrier.replay.chain.size)
      | _ => (0, 0, 0, 0)
    logInfo m!"CAD_{name.getId}_META {carrierDegree} {carrierHeight} {chainHeight} {chainLength} {reflected.sentence.polys.length}"
