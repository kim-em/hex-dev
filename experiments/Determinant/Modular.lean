...
def stages (flat : Bool) (A : Matrix Int n n) : IO Unit := do
  ...
  for p in supply do
    ...
    let (d, t) ← measure reduced (if flat then Matrix.Dixon.flatDet? else Matrix.detMod?)
  ...
  IO.println <| (Lean.Json.mkObj <| times.map (fun (k, v) => (k, Lean.toJson v)) ++
    [("arm", Lean.toJson (if flat then "stages-flat" else "stages")), ("value", Lean.toJson (toString state.value[0])),
