...
def run (kind arm : String) (n bits : Nat) (shape : String) (spread : Nat) : IO Unit := do
  ...
  | "int" =>
    ...
    if arm == "stages" || arm == "stages-flat" then
      return ← Modular.stages (arm == "stages-flat") A
...
def main (args : List String) : IO Unit := do
  ...
  unless arm ∈ ["bareiss", "bird", "berkowitz", "scaled", "modular", "divisor", "stages", "stages-flat"] do
    throw <| IO.userError "unknown arm"
  if (kind == "int" || kind == "poly") && arm == "scaled" ||
      kind == "dyadic" && arm == "bareiss" ||
      kind != "int" && arm ∈ ["modular", "divisor", "stages", "stages-flat"] then
    throw <| IO.userError "unsupported coefficient operation"
