...
namespace Determinant.DivisionAudit

theorem generic {R : Type*} [CommRing R] (a b c d : R) :
    Matrix.det !![a, b; c, d] = a * d - b * c := by division_bird
...
theorem zero {R : Type*} [CommRing R] (a b c d : R) :
    Matrix.det !![a*c, a*d; b*c, b*d] = 0 := by division_bird

theorem characteristicTwo (a b c d : ZMod 2) :
    Matrix.det !![a + b, c; d, a - b] = a^2 - b^2 - c*d := by division_bird
...
theorem symbolicThree {R : Type*} [CommRing R] (a b c d e f g h : R) :
    Matrix.det !![a, b, c; d, 0, e; f, g, h] =
      -a*e*g - b*d*h + b*e*f + c*d*g := by division_bird

theorem integerFour :
    Matrix.det (!![2, 1, 0, 0; 1, 2, 1, 0; 0, 1, 2, 1; 0, 0, 1, 2] :
      Matrix (Fin 4) (Fin 4) Int) = 5 := by division_bird
...
theorem quotientProduct {R : Type*} [Field R] (a b c d u v : R) :
    Matrix.det !![a/u, b/u; c/v, d/v] = (a*d-b*c)/(u*v) := by division_bird

#print axioms quotientRows
#print axioms quotientProduct

theorem nestedQuotient {R : Type*} [Field R] (a d u v : R) :
    Matrix.det !![(a/u)/v, 0; 0, d] = a*d/(u*v) := by division_bird
...
theorem zeroDenominator {R : Type*} [Field R] (a b c d : R) :
    Matrix.det !![(a+b)/0, c/0; c, d] = 0 := by division_bird
...
run_meta do
  ...
  let (_, coeff) ← (Compact.certBirdDet (rα := reified.rα) true true true).run {} |>.run ctx |>.run .reducible
