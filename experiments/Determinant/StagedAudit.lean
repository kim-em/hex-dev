...
namespace Determinant.StagedAudit

theorem generic {R : Type*} [CommRing R] (a b c d : R) :
    Matrix.det !![a, b; c, d] = a * d - b * c := by staged_bird
...
theorem zero {R : Type*} [CommRing R] (a b c d : R) :
    Matrix.det !![a*c, a*d; b*c, b*d] = 0 := by staged_bird

theorem characteristicTwo (a b c d : ZMod 2) :
    Matrix.det !![a + b, c; d, a - b] = a^2 - b^2 - c*d := by staged_bird
...
theorem symbolicThree {R : Type*} [CommRing R] (a b c d e f g h : R) :
    Matrix.det !![a, b, c; d, 0, e; f, g, h] =
      -a*e*g - b*d*h + b*e*f + c*d*g := by staged_bird

theorem integerFour :
    Matrix.det (!![2, 1, 0, 0; 1, 2, 1, 0; 0, 1, 2, 1; 0, 0, 1, 2] :
      Matrix (Fin 4) (Fin 4) Int) = 5 := by staged_bird
...
theorem quotientProduct {R : Type*} [Field R] (a b c d u v : R) :
    Matrix.det !![a/u, b/u; c/v, d/v] = (a*d-b*c)/(u*v) := by staged_bird
