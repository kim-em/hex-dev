...
/-- Computation rule for the recursive Bezout constructor. Keeping this rule
named avoids exposing the dependent eliminator generated by `Cert.Coprime`. -/
theorem checkCoprime_splitBezout {n : Nat} {R : Type u}
    {cmp : Mono (n + 1) → Mono (n + 1) → Ordering}
    {cmp' : Mono n → Mono n → Ordering}
    [Std.TransCmp cmp] [Std.LawfulEqCmp cmp]
    [Std.TransCmp cmp'] [Std.LawfulEqCmp cmp']
    [Lean.Grind.CommRing R] [DecidableEq R] [BEq R] [LawfulBEq R]
    [Dvd R] [GcdOps R] [IsMonomialOrder cmp] [IsMonomialOrder cmp']
    (i : Fin (n + 1)) (f h u v : MvPoly (n + 1) R cmp)
    (r : MvPoly n R cmp')
    (left right : ContentCert n R cmp') (rest : CoprimeCert n R cmp') :
    checkCoprime f h (.splitBezout i cmp' u v r left right rest) =
      (decide (r ≠ 0) &&
        (u * f + v * h == constIn i cmp' r) &&
        checkContent (toUnivariate i cmp' f).toArray.toList left &&
        checkContent (toUnivariate i cmp' h).toArray.toList right &&
        checkCoprime left.value right.value rest) := by
  cases n <;> rfl

/-- Computation rule for the unit constructor. -/
theorem checkCoprime_unit {n : Nat} {R : Type u}
    {cmp : Mono n → Mono n → Ordering}
    [Std.TransCmp cmp] [Std.LawfulEqCmp cmp]
    [Lean.Grind.CommRing R] [DecidableEq R] [BEq R] [LawfulBEq R]
    [Dvd R] [GcdOps R] [IsMonomialOrder cmp]
    (f h : MvPoly n R cmp) :
    checkCoprime f h .unit = (polyIsUnit f || polyIsUnit h) := by
  cases n <;> rfl

/-- Computation rule for a complete gcd certificate. -/
theorem checkGcd_mk {n : Nat} {R : Type u}
    {cmp : Mono n → Mono n → Ordering}
    [Std.TransCmp cmp] [Std.LawfulEqCmp cmp]
    [Lean.Grind.CommRing R] [DecidableEq R] [BEq R] [LawfulBEq R]
    [Dvd R] [GcdOps R] [IsMonomialOrder cmp]
    (f h g cofL cofR : MvPoly n R cmp) (cert : CoprimeCert n R cmp) :
    checkGcd f h (.mk g cofL cofR cert) =
      ((g * cofL == f) && (g * cofR == h) &&
        (polyNormalize g == g) && checkCoprime cofL cofR cert) := by
  cases n <;> rfl
