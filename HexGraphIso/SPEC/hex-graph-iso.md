...
through `not_isomorphic_of_checkKeysP` (`checkKeyP` twice plus
`checkDiff`, with no achieving labelling reified), and its replay cost
scales with the certificate record counts. `checkKeyP`
...
evaluation of the graph's definition per side, and that evaluation is
the floor no negative route undercuts. The pairwise route replays
...
replay is the fallback and anchors the exhaustion semantics. The two
separator legs (`sepRootLitP`, `sepDiffLitP`) replay the same packed
refinement. No result
