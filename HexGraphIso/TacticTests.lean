...
@[expose] def kneser52 : Colored 10 1 :=
  { graph := (Graph.ofEdges? 10
...
@[expose] def c6 : Graph 6 :=
  (Graph.ofEdges? 6 [(0, 1), (1, 2), (2, 3), (3, 4), (4, 5), (0, 5)]).getD
    (Graph.empty 6)

@[expose] def modFallback : Coloring 6 2 := Coloring.mod 6 2 (by omega) (by omega)

@[expose] def edgeMarkA : Colored 6 2 :=
  ⟨c6, (Coloring.ofVector? #v[(0 : Fin 2), 0, 1, 1, 1, 1]).getD modFallback⟩

@[expose] def edgeMarkB : Colored 6 2 :=
  ⟨c6, (Coloring.ofVector? #v[(1 : Fin 2), 1, 1, 0, 0, 1]).getD modFallback⟩

@[expose] def nonedgeMark : Colored 6 2 :=
...
@[expose] def g12relabelled : Colored 12 1 :=
  { graph := Graph.ofAdj
      (fun i j => if i == j then false else
