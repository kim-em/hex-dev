...
public import HexGraphIso.Nauty.Policy.GeneratedComplete
public import HexGraphIso.Nauty.Policy.MaxRoot
import all HexGraphIso.Nauty.Policy.GeneratedComplete
...
theorem root_generates {n k : Nat} (G : Colored n k) (hn0 : 0 < n)
    ...
    Perm.Generated ((Engine.runColoredTraced G).autos.toList.filterMap (autom? G)) p := by
  obtain ⟨last, leaf, hpath⟩ := initial_path G hn0
  have hrun : runState n (rowsOf G) (initialPartition G).1 (initialPartition G).2 =
      Engine.node true { g := rowsOf G } (n + 2) 100 (n + 2) 1 (initialPartition G).2.length
