...
public import HexGraphIso.Nauty.Policy.ReferenceReturn
public import HexGraphIso.Nauty.Policy.MaxResume
...
theorem SweepInput.canon_past {G : Colored n k} {ctx : Ctx n} {tcLevel fuel cfuel : Nat}
  ...
  constructor
  · change (recoverLevels level (recoverPtn (n + 2) level left)).gcaCanon ≤ level
    rw [recover_canon]
    exact Nat.min_le_left _ _
  · intro he
    change (recoverLevels level (recoverPtn (n + 2) level left)).gcaCanon = level at he
    rw [recover_canon] at he
    change min level left.gcaCanon = level at he
    rw [hg] at he
    change ¬ tv < (recoverLevels level (recoverPtn (n + 2) level left)).canonlab[tc]!
    rw [recover_ref, hc]
