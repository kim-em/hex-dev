...
theorem received_trace (first : Bool) (inf level tv1 tv : Nat) (out : Search n) :
    ...
    (recoverLevels level (recoverPtn inf level left)).genTrace = out.genTrace := by
  intro middle left
