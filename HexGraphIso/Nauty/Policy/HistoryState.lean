...
def History (ctx : Ctx n) (tcLevel level agreed numcells : Nat) (st : Search n) : Prop :=
...
theorem History.transport {ctx : Ctx n} {tcLevel level agreed numcells level' agreed' numcells' : Nat}
    {st out : Search n} (h : History ctx tcLevel level agreed numcells st)
    ...
    History ctx tcLevel level' agreed' numcells' out := by
  intro hcheap
  rw [hg] at hcheap ⊢
...
theorem History.classify {ctx : Ctx n} {tcLevel level numcells : Nat} {st : Search n}
    (h : History ctx tcLevel level level numcells st) :
    History ctx tcLevel level level numcells (Engine.classify ctx level numcells st).2 := by
...
def Recorded (level tc : Nat) (st : Search n) : Prop :=
...
theorem Recorded.cheap {level tc : Nat} {st : Search n}
    (h : Recorded level tc st) (first : Bool) (hg : st.gcaFirst ≤ level) :
    Recorded level tc (cheapCheck first level st) := by
  unfold cheapCheck
  split
  · intro hc
...
theorem History.recorded {ctx : Ctx n} {tcLevel level numcells : Nat} {st : Search n}
    ...
    Recorded level r.1.toNat r.2.2.2 := by
...
theorem History.child_return {G : Colored n k} {ctx : Ctx n}
    ...
    History ctx tcLevel level level numcells result ∧ (Recorded level tc st → Recorded level tc result) := by
  ...
  change History ctx tcLevel level level numcells result ∧ (Recorded level tc st → Recorded level tc result)
  constructor
  · intro hcheap
