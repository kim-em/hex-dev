...
theorem cheap_reference (inf tcLevel : Nat)
  ...
  | succ fuel ih =>
    ...
    change SubtreeOk ctx level rs at hS
    ...
    · have hdisc : ∀ q, q < n → rs.ptn[q]! ≤ level := by
        have hc : List.countP (fun q => decide (rs.ptn[q]! ≤ level)) (List.range n) =
          ...
          exact hS.acc.trans hnum
        intro q hq
        simpa using List.countP_eq_length.mp hc q (List.mem_range.mpr hq)
      obtain ⟨hc, hr⟩ := other_leaf (inf := inf) (fuel := fuel) hgsz hnum hS.it hdisc
        hsize hperm (labInj_perm_range hS.it.ok.labSize hS.it.ok.labOk hS.it.inj) hm hleaf hlevel
      refine ⟨hr hguide, ?_⟩
      rw [other_leaf_lab hnum]
      exact hc
    · have hbc : bcount rs.ptn level n < n := by
        rw [hS.acc]
        have hle : rs.numcells ≤ n := hS.acc ▸ bcount_le rs.ptn level n
        omega
      ...
      have hic : IsCell rs.ptn level tc len :=
        cells_isCell (by rw [hS.it.ok.ptnSize]; exact Nat.le_refl _) hS.it.ok.ptnEnd _ hcell
      have hce : cellEnd rs.ptn level (tc + 1) = tc + len - 1 :=
        cellEnd_of_isCell hic (by dsimp only [len]; omega) (by rw [hS.it.ok.ptnSize]; dsimp only [len]; omega)
      ...
      obtain ⟨tv, hnext⟩ := nextElem_windowSet_some (lab := rs.lab) (tc := tc) (len := len)
        (by dsimp only [len]; omega) (hS.it.ok.labOk _ (by rw [hS.it.ok.labSize]; omega))
      obtain ⟨o', ho', hat⟩ := mem_segN_iff.mp (mem_windowSet.mp (VSet.nextElem_mem hnext)).2
      have hocc := hchildLeaf.smallChild hS hlvl hgsz hsymm hloop hcell hne ho
        (by dsimp only [len] at ho'; omega : o' ≤ e - tc)
      have hsmall := subtreeOk_child hS hlvl hsymm hcell hne (by dsimp only [len] at ho'; omega : o' ≤ e - tc)
      ...
      have hstate := other_internal (inf := inf) (fuel := fuel) (hS.acc ▸ hbc) hS.it hS.eqt hm hleaf hlevel hclear
      rw [hstate]
      dsimp only
      rw [hmt]
