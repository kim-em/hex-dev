...
/-- The order of the automorphism group of `G`, by the orbit-stabilizer
chain. It is the order of the automorphism group exactly when each
level's orbit array is the true orbit partition. Without that, each
computed orbit is contained in the true one, so the product is a lower
bound; it is not the order of the group generated by the reported
generators either, since the stabilizer factors come from separate runs
rather than from that list. Conformance pins it against nauty's
`grpsize`. -/
