...
theorem exhaustiveCoreFactorsWithBound_mem_of_scaledRecombinationSearchMod_some
    ...
    {d : Hex.LiftData} {factors : List Hex.ZPoly}
    (hB : B ≠ 0)
    (hd : d = Hex.ZPoly.toMonicLiftData core (Hex.ZPoly.exhaustiveLiftBound core B) primeData)
    (hsearch :
...
theorem exhaustiveCoreFactorsWithBound_coverage_of_henselSubsetCorrespondence_of_bound
    ...
    (h :
      HenselSubsetCorrespondenceHypotheses
        core (Hex.ZPoly.exhaustiveLiftBound core B) primeData d
        admissiblePrime successfulLift)
    (hpartition :
...
theorem toMonicLiftData_subset_of_liftedFactorProduct_map_dvd
  ...
  have hcop :=
    toMonicLiftData_isCoprime_liftedFactorProduct_complement core B primeData
      hcore_lc_pos hcore_pos hselected hprecision T
