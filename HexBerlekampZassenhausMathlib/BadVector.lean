...
/--
Squared-l2-norm bound for the BHKS auxiliary polynomial with diagonal-row
corrections.

This is the summed form of
`auxiliaryPolynomialWithCorrections_coeff_sq_le`: the corrected auxiliary
polynomial pays twice the usual CLD-column Cauchy--Schwarz bound, plus twice
the squared norm of the explicit diagonal correction contribution.
-/
theorem auxiliaryPolynomialWithCorrections_l2norm_sq_le
  ...
  have hsum_le : ∑ j ∈ p.support, (p.coeff j : ℝ) ^ 2 ≤
    ...
    Finset.sum_le_sum_of_subset_of_nonneg hsupport
      (fun j _ _ => sq_nonneg _)
  set vsum : ℝ := ∑ i : Fin r, ((vec.getD i.val 0 : ℝ) ^ 2) with hvsum_def
  have hcoeff_each : ∀ j ∈ Finset.range n,
      (p.coeff j : ℝ) ^ 2 ≤
        2 *
  ...
  have hsum_bd : ∑ j ∈ Finset.range n, (p.coeff j : ℝ) ^ 2 ≤
    ...
    Finset.sum_le_sum hcoeff_each
  have hsum_uncorrected :
      ∑ j ∈ Finset.range n,
          (vsum * ((r : ℝ) * ((Hex.bhksCoeffBound input j : ℝ) ^ 2))) =
        vsum * ((r : ℝ) * ∑ j ∈ Finset.range n,
          ((Hex.bhksCoeffBound input j : ℝ) ^ 2)) := by
  ...
  have hcldcol : (∑ j ∈ Finset.range n, ((Hex.bhksCoeffBound input j : ℝ) ^ 2)) =
    ...
    push_cast
    rfl
  have hsum_split : ∑ j ∈ Finset.range n,
        (2 *
            (vsum * ((r : ℝ) * ((Hex.bhksCoeffBound input j : ℝ) ^ 2))) +
          2 *
            (((corrections.getD j 0 : ℝ) ^ 2) *
          ...
          (∑ j ∈ Finset.range n,
            (((corrections.getD j 0 : ℝ) ^ 2) *
              ((liftData.p : ℝ) ^
                (2 *
                  (liftData.k -
                    Hex.bhksCoeffCutThreshold liftData.p input j))))) := hsum_split
    _ = 2 *
          (vsum * ((r : ℝ) * ∑ j ∈ Finset.range n,
            ((Hex.bhksCoeffBound input j : ℝ) ^ 2))) +
        2 *
        ...
        rw [hsum_uncorrected]
    _ = 2 *
          (vsum * ((r : ℝ) * (BHKS.cldColumnNormBound input liftData.p : ℝ))) +
        2 *
          (∑ j ∈ Finset.range n,
            (((corrections.getD j 0 : ℝ) ^ 2) *
              ((liftData.p : ℝ) ^
                (2 *
                  (liftData.k -
                    Hex.bhksCoeffCutThreshold liftData.p input j))))) := by rw [hcldcol]
    _ = 2 *
          ((∑ i : Fin liftData.liftedFactors.size,
              ((vec.getD i.val 0 : ℝ) ^ 2)) *
            ((liftData.liftedFactors.size : ℝ) *
