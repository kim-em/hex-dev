...
theorem scanDirectCombinations_found
    ...
    ∀ (xs : List (Hex.DirectLiftedIndex basis)) (choose : Nat)
      ...
      Hex.scanDirectCombinations coreLc target basis head xs choose
          selectedRev rejectedRev = .found split tried →
      ∃ selected remaining,
        (selected, remaining) ∈
          Hex.subsetsOfSizeWithComplement xs choose ∧
        split.selected =
  ...
  induction xs with
  | nil =>
      intro choose selectedRev rejectedRev split tried h
      cases choose with
      | zero =>
  ...
  | cons x xs ih =>
      ...
      | succ choose =>
          ...
          generalize hincluded :
              Hex.scanDirectCombinations coreLc target basis head xs choose
                (x :: selectedRev) rejectedRev = included at h
          cases included with
          ...
          | exhausted includedTried =>
              generalize hrejected :
                  Hex.scanDirectCombinations coreLc target basis head xs
                    (choose + 1) selectedRev (x :: rejectedRev) = rejected at h
              cases rejected with
...
theorem scanDirectCombinations_finds
    ...
    (head : Hex.DirectLiftedIndex basis) :
    ∀ (xs : List (Hex.DirectLiftedIndex basis)) (choose : Nat)
      (selectedRev rejectedRev selected remaining :
      ...
      ∃ split tried,
        Hex.scanDirectCombinations coreLc target basis head xs choose
          selectedRev rejectedRev = .found split tried := by
  intro xs
  induction xs with
  | nil =>
      intro choose selectedRev rejectedRev selected remaining
        candidate quotient hmem hworks
      cases choose with
      | zero =>
          simp only [Hex.subsetsOfSizeWithComplement, List.mem_singleton,
            Prod.mk.injEq] at hmem
      ...
      | succ choose =>
          ...
          · simp only [Prod.mk.injEq] at hshape
            ...
            obtain ⟨split, tried, hfound⟩ :=
              ih choose (x :: selectedRev) rejectedRev selected remaining
                candidate quotient hmem hworks'
          ...
          · simp only [Prod.mk.injEq] at hshape
            ...
            generalize hincluded :
                Hex.scanDirectCombinations coreLc target basis head xs choose
                  (x :: selectedRev) rejectedRev = included
            cases included with
            | found split tried =>
            ...
            | exhausted triedLeft =>
                ...
                obtain ⟨split, tried, hfound⟩ :=
                  ih (choose + 1) selectedRev (x :: rejectedRev)
                    selected remaining candidate quotient hmem hworks'
