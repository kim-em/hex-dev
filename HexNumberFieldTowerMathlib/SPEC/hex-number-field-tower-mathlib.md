...
## Real sign correspondence

For the [tower sign extension](../../HexNumberFieldTower/SPEC/hex-number-field-tower.md#real-sign-and-comparison),
prove the following against the existing fixed embedding `T.toComplex`:

- `isReal_iff` identifies the real-tower check with reality of every selected
  generator. This does not assert that every embedding of the field is real.
- `signEliminant_spec` proves that the normalized absolute norm of `S-x` is
  nonzero, has degree at most `T.dim`, and annihilates the selected evaluation.
  Removing powers of `X` then supplies the reciprocal-Cauchy lower bound for
  nonzero evaluations.
- `signBall_bound` proves membership and radius at most `C_v*2^-k` by induction
  on the mixed-radix coefficient tree. Include the child's
  `k + ceilLog2 C_w` precision shift; an unshifted recursive coefficient ball
  need not meet the parent's one-unit error hypothesis.
- `sign?_isSome` and `compare?_isSome` establish the prescribed finite precision
  endpoint and retire the total wrappers' invariant-failure branches. They
  require the real-tower or individual real-element witnesses stated by the
  computational owner; no order is installed on arbitrary complex towers.
- `sign_eq` and `compare_eq` identify the results with `realCompare` on the
  canonical materializations `A_T(x)` specified by the owner. Combine norm
  annihilation, injective coordinate zero, the ball bound, and the existing
  `evalCoords_sound`/evaluation-success bridges. Materialization is reference
  semantics and is not executed by the sign evaluator.

The empty tower specializes to rational order. Real elements of a complex
tower retain their own reality premise. These new obligations extend the
shipped surface; conformance and benchmark ownership stays in
`HexNumberFieldTower`, including the separately measured cost of constructing
individual reality witnesses.
