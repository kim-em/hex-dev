...
theorem pseudo_reconstruct (p q : DensePoly E) (hq : q ≠ 0) :
...
theorem pseudo_rem (p q : DensePoly E) (hq : q ≠ 0) :
  ...
  have h := congrArg (fun r => r % interpret f hz q) (pseudo_reconstruct f hz h1 ha hs hm p q hq)
...
theorem pseudo_divMod (p q : DensePoly E) (hq : q ≠ 0) :
  ...
  · have hq' : interpret f hz q ≠ 0 := fun h => hq ((interpret_eq_zero f hz q).mp h)
    ...
    have hrec := pseudo_reconstruct f hz h1 ha hs hm p q hq
    have hrem := pseudo_rem f hz h1 ha hs hm p q hq
  ...
  · exact pseudo_rem f hz h1 ha hs hm p q hq
...
private theorem pseudo_dvd_step (p q : DensePoly E) (hq : q ≠ 0) (d : Polynomial K)
  ...
  rw [← pseudo_reconstruct f hz h1 ha hs hm p q hq] at h
...
theorem positive_reconstruct [Neg E] (hn : ∀ a, f (-a) = -f a)
    (sign : E → Int) (p q : DensePoly E) (hq : q ≠ 0) :
  ...
  · simp only [hn, Polynomial.C_neg, neg_mul, interpret_neg f hz hs, ← neg_add]
    exact congrArg Neg.neg (pseudo_reconstruct f hz h1 ha hs hm p q hq)
  · exact pseudo_reconstruct f hz h1 ha hs hm p q hq
...
theorem positive_multiplier [Neg E] [LinearOrder K] [IsStrictOrderedRing K]
  ...
  split
  · rename_i h
    simpa only [hn] using neg_pos.mpr ((hsign _).mp h)
  · rename_i h
    have hle : 0 ≤ f (pseudoDiv p q).multiplier :=
      not_lt.mp (fun hneg => h ((hsign _).mpr hneg))
