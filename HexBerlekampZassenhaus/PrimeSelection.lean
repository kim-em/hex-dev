...
/-- The window of committed table primes in `[lo, hi)`, as proof-carrying
candidates: primality from `Hex.Nat.mem_primeTable_prime`, the
`ZMod64.Bounds` instance from the window bound `hhi`. -/
@[expose]
  ...
  (Hex.Nat.primeTable.toList.filter
    ...
    fun ⟨q, hq⟩ =>
      ...
      have hlt : q < 2 ^ 31 := by
        ...
        omega
      { p := q, bounds := { pPos := hprime.pos, pLtR := hlt }, prime := hprime }
...
private theorem exists_mem_tableCandidates {lo hi : Nat} {hhi : hi ≤ 2 ^ 31}
    ...
    ∃ c ∈ tableCandidates lo hi hhi, c.p = p := by
...
private theorem tableCandidates_map_p {lo hi : Nat} {hhi : hi ≤ 2 ^ 31} :
    (tableCandidates lo hi hhi).map (fun c => c.p) =
      Hex.Nat.primeTable.toList.filter
        (fun q => decide (lo ≤ q) && decide (q < hi)) := by
...
/-- The default list of small primes (`3` through `71`) used for
Berlekamp-Zassenhaus trial division: the committed table window `[3, 72)`. -/
def smallPrimeCandidates : List SmallPrimeCandidate :=
  tableCandidates 3 72 (by decide)

/-- The extended list of larger small-prime candidates (`73` through `499`),
tried when `smallPrimeCandidates` is exhausted: the table window `[72, 501)`.
(`500` itself is composite, so the window's closed upper edge is inert.) -/
...
theorem hotPathPrimeValues_nodup :
  ...
  have hsorted : (hotPathCandidates.map fun c => c.p).Pairwise (· < ·) := by
    unfold hotPathCandidates smallPrimeCandidates extendedSmallPrimeCandidates
    rw [List.map_append, tableCandidates_map_p, tableCandidates_map_p,
...
prime in the closed range `[3, 500]`. The `Hex.Nat.Prime` conjunct is the
structure field directly; the bounds come from the two table windows.
-/
...
`3 ≤ p ≤ 500` appears as the `.p` field of some candidate, as a corollary
of the committed table's completeness direction.
-/
