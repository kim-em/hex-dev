...
* `runFactorFallbackProbeChecksum`: public `factor` combinator on the explicit
  cascade-trigger split-degree ladder `fallbackProbeSchedule`.
...
/-- Cascade-trigger split-degree ladder from the BZ-vs-Isabelle post-mortem. -/
