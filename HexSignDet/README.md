...
their retained row bases. Every node checks its shared Tarski certificates and
HexRank certificate. Retained columns are computed by removing exactly the
...
a hash or reused numeric identifier is insufficient. The checker reconstructs
unreduced moment products and runs no query producer, gcd search or row search.
