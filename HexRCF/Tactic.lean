...
private meta partial def checkAxiomPath (handler constant : Name) :
  ...
  if let some moduleName := admittedModule constant then
    ...
    -- The imported module's dependency inventory must contain the exact
    -- admitted `sorryAx`; a source declaration changed to an axiom has only
    -- its own name in that inventory and must be rejected.
