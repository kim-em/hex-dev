...
3. **Claim the issue explicitly** with
   ...
   **Exception — `WORKER TYPE: replan`.** Replan does not claim.
   `coordination claim` deliberately *rejects* `replan`-labelled
   issues (`CLAIM FAILED: Issue #<N> needs replan`), and the replan
   skill triages by editing the issue directly (no claim, no PR).
   For replan, skip the claim; instead confirm the issue appears in
   `coordination list-replan` and follow the `replan` skill. Treat a
   `coordination list-replan` miss (not a `coordination claim`
   failure) as the abort signal for step 4.
4. **If the claim fails** (issue already claimed by another agent,
   ...
   `replan`, the analogous abort is the issue being absent from
