...
{name}`Hex.AlgebraicRoot.exact`. A {name}`Hex.QAdjoin` is the field `ℚ(a)`
generated by one canonical number, with elements written as rational
coordinates in the power basis; arithmetic there is polynomial
arithmetic modulo the minimal polynomial and involves no root isolation at
