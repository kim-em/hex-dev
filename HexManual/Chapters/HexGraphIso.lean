...
{docstring Hex.GraphIso.Mathlib.autEquiv}

The decoded list {name Hex.GraphIso.Mathlib.autos}`autos e G` contains
colour-preserving self-isomorphisms. Its completeness theorem is stated
directly with Mathlib's {name Subgroup.closure}`Subgroup.closure`: every
element of `Colored.Iso G G` belongs to the subgroup generated by that
list.

{docstring Hex.GraphIso.Mathlib.autos}

{docstring Hex.GraphIso.Mathlib.autos_complete}

The reported orbit array also describes the action of the full group,
not merely the action of the generators encountered before a particular
point in the search. Equality of two stored representatives is
equivalent to the existence of a colour-preserving self-isomorphism
carrying one vertex to the other. The quotient equivalence exposes the
same fact in Mathlib's group-action vocabulary.

{docstring Hex.GraphIso.Mathlib.autos_sameOrbit}

{docstring Hex.GraphIso.Mathlib.autOrbitEquiv}

Finally, the two numerical results have exact cardinality theorems.
`autNumOrbits e G` is the cardinality of the orbit quotient for the
action of `Colored.Iso G G` on `V`, and `autOrder e G` is the cardinality
of `Colored.Iso G G` itself.
