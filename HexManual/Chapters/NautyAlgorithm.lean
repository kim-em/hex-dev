...
## The automorphism output

The automorphisms of the previous section are not only a pruning
device. They are the program's second output, and which ones it
reports is as much part of the specified function as the canonical
form is, so this section says exactly what is emitted, in what order,
and how the orbits and the group order follow from them.

Two kinds of leaf produce an automorphism. The first is a leaf whose
refinement codes agree with the first leaf's along the whole path from
the root: the permutation carrying the first leaf's labelling onto this
one is then tested, and reported when it is an automorphism. The
second is a leaf that ties the best leaf found so far under the key
order: the permutation carrying the best labelling onto this one is an
automorphism by construction, since the two leaves relabel the graph
identically, and it is reported as well. No other leaf produces one.

The order of the report is the order of discovery along the traversal
described above. The traversal is deterministic, and the pruning rules
decide which leaves are ever visited, so the reported list is a
function of the input graph and colouring, and not only of the group.
Two presentations of the same graph will generally report different
lists.

The vertex orbits are derived from the reported permutations alone. A
single array over the vertices starts as the identity. Joining it with
a permutation walks each vertex and its image up to their current
representatives and points the larger representative at the smaller; a
final pass replaces every entry by its representative, so the array is
flat when the join returns. The array after the last join is the
reported orbit array, and the number of orbits is the number of
vertices that are their own representative. Note that the orbits
reported are the orbits of the group generated by the reported
permutations, which is a subgroup of the automorphism group; the claim
that the two coincide is not part of this description.

The order of the automorphism group is derived by the orbit-stabilizer
relation. If some orbit has more than one vertex, pick one such vertex,
give it a colour of its own placed after all existing colours, and run
the whole traversal again on that colouring. A colour-preserving
...
traversal per level. Generation completeness proves that every level's
reported orbit array is the full automorphism orbit partition. The
Mathlib bridge follows this recursion to prove that the product is the
cardinality of the full group; a discrete orbit partition means that
every automorphism fixes every vertex and is the identity.
...
the second kind, the program joins the orbit array with the new
permutation first, and reports the permutation only if the join reduced
the number of orbits; a tie automorphism that adds nothing to the orbit
partition is used for pruning and then discarded. Leaves of the first
kind are always reported.
...
{name Hex.GraphIso.Aut.trace}`Aut.trace` reads it off,
{name Hex.GraphIso.autom?}`autom?` rebuilds each entry as a permutation
of the vertices and accepts it only when it passes the isomorphism
check, and {name Hex.GraphIso.autos}`autos` packages the checked list
with the orbit array, the orbit count and the group order. The orbit
array is built with the search's own
{name Hex.GraphIso.Nauty.orbjoin}`orbjoin`, so it is the array the
program reports rather than a recomputation.

{docstring Hex.GraphIso.Aut.trace}

{docstring Hex.GraphIso.autom?}

{docstring Hex.GraphIso.Nauty.orbjoin}
...
{name Hex.GraphIso.autos_isIso}`autos_isIso` says every reported
permutation is an automorphism, and
{name Hex.GraphIso.autos_complete}`autos_complete` says every automorphism
is generated by the returned list.
{name Hex.GraphIso.autos_sameOrbit}`autos_sameOrbit` identifies equality
...
{docstring Hex.GraphIso.autos_complete}

{docstring Hex.GraphIso.autos_isIso}

{docstring Hex.GraphIso.autos_sameOrbit}
