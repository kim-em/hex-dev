...
The automorphism output carries two theorems and one open contract.
{name Hex.GraphIso.autos_isIso}`autos_isIso` says every reported
permutation is an automorphism, and
{name Hex.GraphIso.autos_sameOrbit}`autos_sameOrbit` says two vertices
sharing an orbit representative are carried onto each other by one.
That the reported permutations generate the whole automorphism group,
which is what would make the orbit count and the group order theorems
rather than measurements, is not proved; conformance compares both
against nauty exactly.
