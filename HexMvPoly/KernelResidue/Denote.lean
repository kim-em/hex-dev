...
/-! Producer conversions and executable denotation of natural-residue lists.
These definitions are outside the closed Boolean certificate path. -/
...
/-- Reading the modular identity gives the executable list identity. -/
theorem toResidues_oneMod (n : Nat) :
    toResidues p (oneMod p n) = one (κ := Hex.ZMod64 p) n := by
  change toResidues p (oneMod p n) =
    if Hex.ZMod64.ofNat p 1 = Hex.ZMod64.ofNat p 0 then []
    else [(zeroExp n, Hex.ZMod64.ofNat p 1)]
  have hz : Hex.ZMod64.ofNat p 1 = Hex.ZMod64.ofNat p 0 ↔ 1 % p = 0 := by
    rw [Hex.ZMod64.eq_iff_toNat_eq,
      Hex.ZMod64.toNat_ofNat, Hex.ZMod64.toNat_ofNat]
    rfl
  by_cases hp : 1 < p
  · have hne : Hex.ZMod64.ofNat p 1 ≠ Hex.ZMod64.ofNat p 0 := by
      intro he
      have h := hz.mp he
      rw [Nat.mod_eq_of_lt hp] at h
      contradiction
    simp [oneMod, Nat.blt_eq.mpr hp, hne, toResidues, map]
  · have hp1 : p = 1 := by have := Hex.ZMod64.Bounds.pPos (p := p); omega
    have he : Hex.ZMod64.ofNat p 1 = Hex.ZMod64.ofNat p 0 := hz.mpr (by simp [hp1])
    simp [oneMod, Nat.blt_eq, hp, he, toResidues, map]
...
/-- The residue-list identity denotes the polynomial identity. -/
@[simp] theorem denoteMod_oneMod : denoteMod p (cmp := cmp) (oneMod p n) = 1 := by
  unfold denoteMod
  rw [toResidues_oneMod]
  exact denote_one
...
theorem ofResidues_toList_canonical (a : Hex.MvPoly n (Hex.ZMod64 p) cmp) :
