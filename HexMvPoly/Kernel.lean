...
def expBeqImpl : List Nat → List Nat → Bool
  | [], [] => true
  | a :: as, b :: bs => Nat.beq a b && expBeqImpl as bs
  | _, _ => false

/-- Kernel equality through a simple recursor; compiled calls use `expBeqImpl`. -/
noncomputable def expBeq (a b : List Nat) : Bool :=
  List.rec (motive := fun _ => List Nat → Bool)
    (fun b => match b with | [] => true | _ :: _ => false)
    (fun x _ next b => match b with
      | [] => false
      | y :: ys => Nat.beq x y && next ys) a b

/-- Lexicographic comparison of exponent lists, using only `Nat.blt`. -/
def expCmpImpl : List Nat → List Nat → Ordering
  | [], [] => .eq
  | [], _ :: _ => .lt
  | _ :: _, [] => .gt
  | a :: as, b :: bs =>
      ...
      else expCmpImpl as bs

/-- Kernel comparison through a simple recursor; compiled calls use `expCmpImpl`. -/
noncomputable def expCmp (a b : List Nat) : Ordering :=
  List.rec (motive := fun _ => List Nat → Ordering)
    (fun b => match b with | [] => .eq | _ :: _ => .lt)
    (fun x _ next b => match b with
      | [] => .gt
      | y :: ys =>
          if Nat.blt x y then .lt
          else if Nat.blt y x then .gt
          else next ys) a b
...
def addExpImpl : List Nat → List Nat → List Nat
  | [], bs => bs
  | as, [] => as
  | a :: as, b :: bs => Nat.add a b :: addExpImpl as bs

/-- Kernel exponent addition through a simple recursor; compiled calls use `addExpImpl`. -/
noncomputable def addExp (a b : List Nat) : List Nat :=
  List.rec (motive := fun _ => List Nat → List Nat)
    (fun b => b)
    (fun x xs next b => match b with
      | [] => x :: xs
      | y :: ys => Nat.add x y :: next ys) a b

@[simp] theorem expBeq_nil (b : List Nat) :
...
def mulTermImpl [Zero κ] [Add κ] [Mul κ] [DecidableEq κ]
    (t : Term κ) : PolyList κ → PolyList κ
  | [] => []
  | u :: us =>
      insert (addExp t.1 u.1, t.2 * u.2) (mulTermImpl t us)

/-- Kernel row multiplication; compilation uses the equation implementation. -/
...
@[csimp] theorem mulTerm_eq_impl : @mulTerm = @mulTermImpl := by
  funext κ _ _ _ _ t p
  induction p with
  | nil => rfl
...
def beqImpl [DecidableEq κ] : PolyList κ → PolyList κ → Bool
  | [], [] => true
  | t :: ts, u :: us =>
      expBeq t.1 u.1 && decide (t.2 = u.2) && beqImpl ts us
  | _, _ => false

/-- Kernel list equality; compilation uses the equation implementation. -/
...
@[csimp] theorem beq_eq_impl : @beq = @beqImpl := by
  funext κ _ p q
...
theorem expCmp_eq_compare (a b : List Nat) : expCmp a b = compare a b := by
  ...
  | cons x xs ih =>
      ...
      | cons y ys =>
          by_cases hxy : x < y
          · simp [Nat.blt_eq.mpr hxy, Nat.compare_eq_lt.mpr hxy]
          · by_cases hyx : y < x
            · simp [Nat.blt_eq, hxy, hyx, Nat.compare_eq_gt.mpr hyx]
...
theorem expCmp_eq_lt_iff {a b : List Nat} : expCmp a b = .lt ↔ a < b := by
  ...
  | cons x xs ih =>
      ...
      | cons y ys =>
          by_cases hxy : x < y
          · simp [Nat.blt_eq.mpr hxy, List.cons_lt_cons_iff, hxy]
          · by_cases hyx : y < x
            · have hne : x ≠ y := Nat.ne_of_gt hyx
              simp [Nat.blt_eq, hxy, hyx, hne, List.cons_lt_cons_iff]
...
theorem length_addExp {a b : List Nat} {n : Nat}
  ...
  | cons x xs ih =>
      ...
      | cons y ys =>
          ...
          simp only [addExp_cons_cons, List.length_cons]
          rw [hi]
          omega
...
/-- Materialize the input once instead of repeatedly indexing a linked list. -/
def monoFast (n : Nat) (e : List Nat) : Mono n :=
  let entries := e.toArray
  Hex.Vector.ofFn' fun i => (entries[i.val]?).getD 0

@[csimp] theorem mono_eq_fast : mono = monoFast := by
  funext n e
  apply Vector.ext
  intro i hi
  simp [mono, monoFast, List.getD]
...
theorem getD_addExp {a b : List Nat} {n i : Nat}
  ...
  | cons x xs ih =>
      ...
      | cons y ys =>
          ...
          | succ i =>
              simp only [List.length_cons] at ha hb
              simp only [addExp_cons_cons, List.getD_cons_succ]
...
theorem denote_mulTerm {n : Nat} {κ : Type u} [Lean.Grind.Semiring κ]
  ...
  induction p with
  | nil =>
      simp only [mulTerm_nil, denote]
      exact (MvPoly.mul_zero _).symm
  | cons u us ih =>
      rw [mulTerm_cons, denote_insert, ih
...
theorem beq_eq_true_iff [DecidableEq κ] {p q : PolyList κ} :
  ...
  | cons t ts ih =>
      ...
      | nil => simp [beq]
      | cons u us =>
          simp only [beq_cons_cons, Bool.and_eq_true, decide_eq_true_eq, ih, expBeq_iff]
          constructor
          · rintro ⟨⟨he, hc⟩, htail⟩
