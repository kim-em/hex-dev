...
uses `Nat.add`. Their kernel definitions use `List.rec` directly, avoiding
the `below` tuples generated by course-of-values recursion, as do term-list
equality and multiplication by a single term. Proved `@[csimp]`
equalities retain the ordinary recursive implementations for compiled callers,
including the total behavior on exponent lists of unequal lengths.
Coefficient computation uses the representation's ordinary
