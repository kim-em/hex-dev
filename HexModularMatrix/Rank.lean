...
def candidate (A : Matrix Int n m) (fuel : Nat) (q : ZMod64.Prime) :
  ...
  let d := (Hex.ModularMatrix.ordinaryWith B fuel).value
...
def search (A : Matrix Int n m) (fuel : Nat) : List ZMod64.Prime →
    Option (RankCert Int n m)
  | [] => none
  | q :: qs =>
    match candidate A fuel q with
    | some c => if checkRank A c then some c else search A fuel qs
    | none => search A fuel qs

theorem search_check {A : Matrix Int n m} {fuel : Nat} {qs : List ZMod64.Prime}
    {c : RankCert Int n m} (h : search A fuel qs = some c) : checkRank A c = true := by
  induction qs with
...
def rankCert? (A : Matrix Int n m) (fuel : Nat) : Option (RankCert Int n m) :=
  if fuel = 0 then none
  else ModularRank.search A fuel (ZMod64.primesBelow (2 ^ 31 - 1) fuel).toList
