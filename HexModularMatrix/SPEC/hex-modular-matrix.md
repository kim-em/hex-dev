...
[future-work](../../SPEC/future-work.md) opens with a warning that checking a
...
multi-modular form [hex-char-poly](../../SPEC/Libraries/hex-char-poly.md) lists among its
...
[hex-padics](../../SPEC/Libraries/hex-padics.md) records that the residual vector stays an
...
**Checked inversion is in hex-mod-arith.** hex-mod-arith's `ZMod64.inv` in
...
The definition and lemma live beside `inv`.

**The CRT congruence and uniqueness lemmas are in hex-modular.** `CrtTrace.congr` (a traced state's
...
characterisation. `det_mapEntries` in hex-determinant proves that the
Leibniz determinant commutes with a ring homomorphism and underlies
`detMod?_reduce`.

**Natural square roots are in hex-arith.** `HexArith.Nat.floorSqrt` and
`ceilSqrt` live in `HexArith/Nat/Sqrt.lean`, with compatibility aliases in
`HexPolyZ/Mignotte.lean`. `hadamardBound` uses them without a dependency
