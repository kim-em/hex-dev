...
| `kronecker` | `a = b` for commutative-ring expressions in any characteristic | `hex-kronecker`: `Expr`, `checkExprEq`; quotient-witness checks are programmatic only | `hex-kronecker-mathlib`: `checkExprEq_sound`, `kronecker`, `kronecker%` | planned, explicitly opt-in: [hex-kronecker-mathlib](../HexKroneckerMathlib/SPEC/hex-kronecker-mathlib.md) |
