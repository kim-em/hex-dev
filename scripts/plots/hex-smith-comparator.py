#!/usr/bin/env python3
"""Render HexSmith Lean/FLINT/PARI fixed-comparator runtime plots."""
...
DEFAULT_INPUT = (
    ...
    / "reports"
    / "bench-results"
    / "hex-smith-phase4-comparators.json"
)

FAMILIES: dict[str, tuple[str, list[int]]] = {
    "random-dense-smith": ("Dense", [6, 8, 10, 12, 14]),
    "chain-conjugate": ("Chain", [6, 8, 10, 12, 14]),
    "presentation-smith": ("Presentation", [8, 12, 16, 20, 24]),
}
...
def _load_family(
    ...
    prefix, parameters = FAMILIES[family]
    ...
    return eligible_parameters, series
...
def _render(family: str, parameters: list[int], series: dict[str, list[float]]) -> str:
    ...
    out = [
        ...
        f'<text x="{width / 2}" y="25" text-anchor="middle" font-family="sans-serif" font-size="17">HexSmith comparator runtimes: {family}</text>',
    ]
...
def main() -> int:
    ...
    output = args.output or (
        REPO_ROOT / "reports" / "figures" / f"hex-smith-comparator-{args.family}.svg"
    )
