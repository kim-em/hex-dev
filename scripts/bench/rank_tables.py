...
def parametric(root, directories, output, quotient=False):
    rows = []
    ...
    intro += 'The linked exports retain every point. Mode-2 faster results translate the harness’s `inconclusive` result using the declared one-sided bound; they are not two-sided passes. The original resolution failures remain in their original directories.\n\n'
    (root / output).write_text(intro + '| Case | Mode | Declared model | β | Eligible rungs | Result |\n| --- | ---: | --- | ---: | ---: | --- |\n' + '\n'.join(rows) + '\n')


def polynomial(root, analysis):
    rows = []
...
def main():
    ...
    parametric(root, ['integer', 'attribution'], 'integer-verdicts.md')
    parametric(root, ['quotient-large/' + op + rank for rank in ('Full', 'Deficient')
                     for op in ('produce', 'prepare', 'finish', 'check')], 'quotient-verdicts.md', True)
    polynomial(root, analysis)
    ratios(root, analysis)
