...
/-- The canonical polynomial list operations used by the checker. -/
@[expose] def ops (k : Nat) : DetOps (Hex.MvPoly.Kernel.PolyList C) where
  zero := []
  one := Hex.MvPoly.Kernel.one k
  add := Hex.MvPoly.Kernel.add
  mul := Hex.MvPoly.Kernel.mul
  neg := Hex.MvPoly.Kernel.neg
  beq := Hex.MvPoly.Kernel.beq
  valid := Hex.MvPoly.Kernel.isCanonical k
...
noncomputable def decode (k : Nat) : Decode (ops (C := C) k) (MvPolynomial (Fin k) C) where
  ...
  zero := by simp [ops, HexMvPolyMathlib.Kernel.denote, Hex.MvPoly.Kernel.denote]
...
/-- The batch's quoted matrix evaluated at its atom context. -/
noncomputable abbrev evaluated (k n : Nat)
  ...
  (HexMatrixMathlib.matrixEquiv (matrix k n rows)).map (HexReflectMathlib.Kernel.hom k ctx)
...
theorem target (k n : Nat) (rows : List (List (Hex.MvPoly.Kernel.PolyList Int)))
    ...
    (hcheck : checkDetPolyList (ops k) n rows w = true)
    (hA : A = (HexMatrixMathlib.matrixEquiv (matrix k n rows)).map
  ...
  rw [transport k n rows w (HexReflectMathlib.Kernel.hom k ctx) A hcheck hA,
...
theorem scaled (k n : Nat) (rows : List (List (Hex.MvPoly.Kernel.PolyList Int)))
    ...
    (hcheck : checkDetPolyList (ops k) n rows w = true)
    (hA : (HexMatrixMathlib.matrixEquiv (matrix k n rows)).map
      (HexReflectMathlib.Kernel.hom k ctx) =
  ...
  have hdet := transport k n rows w (HexReflectMathlib.Kernel.hom k ctx)
    (Matrix.diagonal (fun i : Fin n => (s.getD i 1 : Rat)) * A) hcheck hA.symm
