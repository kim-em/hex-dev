...
default enablement follow that contract and its benchmark gates. ECM stage 2 has the separate bounded coordinate-collision contract below.
...
direct-`Nat` route is useful before this milestone is complete. The bounded ECM stage-2 route below uses direct `Nat` arithmetic; Pollard p−1 stage 2 uses its own
...
`Hex.Nat.Ecm`. `start n sigma B₁` returns the stage-1 result and, only after
...
change. The core provider runs first. If its known factor product squared
...
entries are multiplied back into the residual. Thus all work is bounded,
...
`HexIntFactor.Construction`. The measured named-prime successes, exact emitted
certificate guards, ordinary replay, and fixed native construction/checker
