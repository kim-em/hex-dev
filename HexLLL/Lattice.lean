...
/-!
Linear independence of the rows of an integer matrix, and the row-operation
lemmas showing that an adjacent swap or a size-reduction row combination
preserves the generated lattice.

Membership itself is `Hex.Matrix.memLattice` in `hex-matrix`, with its
transport lemmas. The operation-specific lemmas stay here because they need
the row-operation multiplicative laws from `hex-row-reduce`.
-/
