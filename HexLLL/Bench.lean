...
def getCachedInput {α : Type} (ref : IO.Ref (Option α)) (mk : Unit → α) : IO α := do
  match (← ref.get) with
...
def gramSchmidtCoeffLoopIters : Nat := 327_680
...
/-- Family-specific model for bounded random bases. These fixtures trigger few
swaps, so the measured hot path is the `n` size-reduction passes, each doing a
triangular row/coefficient update, with a logarithmic determinant-width factor
from the stored `d`/`ν` integers. -/
def firstShortVectorRandomBoundedComplexity (n : Nat) : Nat :=
  n ^ 3 * Nat.log2 (n + 1)

/-- Family-specific model for harsh-cubic inputs. The same size-reduction
surface dominates, but the fixture's `~3.3n`-bit entries add one linear
operand-width factor on top of the logarithmic determinant-width factor. -/
def firstShortVectorHarshCubicComplexity (n : Nat) : Nat :=
  n ^ 4 * Nat.log2 (n + 1) * Nat.log2 (n + 1)
...
/-- Benchmark target: repeat one stored coefficient projection enough times to
clear the child-process signal floor. The wrapped operation is still
dimension-independent; the fixed loop count is measurement overhead only. -/
def runGramSchmidtCoeffLoopChecksum (input : StateInput) : Int :=
  (List.range gramSchmidtCoeffLoopIters).foldl
    (fun acc _ => acc + runGramSchmidtCoeffChecksum input)
    0
...
The benchmark repeats that projection a fixed number of times only to clear the
child-process signal floor; the declared model remains constant in `n`. -/
setup_benchmark runGramSchmidtCoeffLoopChecksum n => gramSchmidtCoeffComplexity n
  with prep := prepStateInput
  where {
    paramFloor := 32
    paramCeiling := 128
...
/- Complexity derivation: random-bounded inputs have square dimension `n`,
bounded entries, and the committed seed exercises size reduction with few
swaps. The timed path is therefore the cubic size-reduction surface, multiplied
by a logarithmic determinant-width factor from the stored integer `d`/`ν`
values. -/
...
paper regime named in `phase4.input_families`. The same few-swap
size-reduction surface as the random-bounded family dominates, with an extra
linear operand-width factor from the fixture entries and a logarithmic
determinant-width factor from the stored integer state; the second logarithmic
factor captures the bigint multiplication/division cost across the stored
coefficient updates. -/
