...
**5.18 ms versus 13.98 ms**; Curve448 takes **10.29 ms versus 34.11 ms**.
The **2% margin at 31 bits is narrow** and should be treated as provisional
on this shared host. The other margins range from 1.53× to 3.32×.

PrimeCert uses compact certificates with the same selected Pocklington factors
as Hex. This is a stronger comparator than the original, overfactored fixtures.
...
|---|---:|---:|---:|
| family-31 | 0.563 | 0.576 | 1.02× |
| family-61 | 0.498 | 0.765 | 1.53× |
| family-123 | 0.764 | 1.552 | 2.03× |
| family-256 | 1.269 | 3.273 | 2.58× |
| family-511 | 2.629 | 6.708 | 2.55× |
| family-512 | 3.038 | 10.097 | 3.32× |
| Curve25519 | 5.182 | 13.980 | 2.70× |
| Curve448 | 10.287 | 34.114 | 3.32× |
...
[complete record](bench-results/hex-primality-small-replay/hex-compact-primecert-six-bit-kernel.json)
...
isolated comparison on an identical kernel. Native decision and complete fresh
...
- `HexArith.powModNat` uses direct recursors and fixed windows: six bits through
  ...
  including separate bases `2`, `97`, and `n - 2`. The certificate corpus does
...
The public checkers retain their accepted results. Compiled modular arithmetic
...
PrimeCert's imported small-prime proofs stop at 997. Hex table leaves above
that become additional checked Pocklington nodes in PrimeCert; they are not
silently treated as imported proofs. Every generated source was built in the
unmodified PrimeCert checkout before timing.
...
including the pinned axiom diagnostic.
...
- `lake build`: all 14,400 jobs pass, including the revised manual.
...
- Existing HexArith, HexPrimality, and HexIntFactor benchmark verification
  passes within the wrapper's budget (19 seconds on this host).
