...
record itself contains every raw null and substantive sample; its aggregate
results and SHA-256 are filled after the designated host accepts a complete
strict run.  The exact command is:

```sh
python3 scripts/bench/primality_core_proof_sweep.py --samples 6 \
  --shared-host --expected-host chungus2 --cpu 7 --timeout 30 \
