...
Over the fitted 16-fold parameter range, the factors `s` and
`s(log₂s+1)` differ in normalized slope by only
`log(12/8)/log(2048/128) ≈ 0.146`, below the pinned `0.15` tolerance.
Consequently this verdict does not empirically distinguish those models for
`runProduce`, `runDirect` or `runTree`; the derived arity-volume count supplies
their asymptotic argument. The `runGraph` alternative is just outside the
tolerance. A wider parameter sweep is needed to resolve the log factor
reliably from timings alone. Each timed input is generated by the same pure
`input` function and binary as its untimed checked inventory; LeanBench's
export does not carry an input hash of its own.
...
The [current summary](data/sign-det-sparse/profile-current-9f4b091ad/sparse-support.summary.json)
and [manifest](data/sign-det-sparse/profile-current-9f4b091ad/sparse-support.manifest.json)
profile `runProduce` at `s=2048` on the same binary SHA-256
`0c052279d2d38d5f04b9d30e53d09b694c16014ad32ef28e3d4464067b2f4907`
as both current timing schedules. The clean source is `9f4b091ad`, differing
from the timed source only in CI's bench-registration count and report data.
The run used shared host `chungus2`, automatically leased CPU 92, and retained
4,158 samples within 4.164 seconds of timed regions. Calibration residual was
0.914 ms against a 5 ms limit; confidence and boundary-shift checks passed,
with no off-thread samples in those regions. Leaf self-time was 37.71%
allocation/free, 20.47% GMP, 28.43% Lean runtime, 5.32% Lean own code and
8.08% other. The main inclusive functions were `buildPrepared` (95.50%),
`Replay.check` (53.25%) and `buildTreeFrom` (42.78%); these overlap and must
not be summed. This is allocation **time**, not allocated-byte evidence.
The raw files are retained locally at
`/home/kim/.local/share/hex-bench/sign-det-sparse-profile-current-9f4b091ad`;
all 35 files match the committed manifest hashes.

## Earlier sparse-support profile
...
This earlier profile uses a different binary and source revision; its
attribution below is retained for provenance, not used as current-source
attribution.
