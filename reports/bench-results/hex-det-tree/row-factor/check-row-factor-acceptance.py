...
output = root / 'row-factor-acceptance-comparison.json'
observations = root / 'row-factor-acceptance-observations.jsonl'
...
def run(deadline):
    ...
    sys.argv = ['det_symbolic_sweep.py', str(output), '--case', 'N8K4D1S1', '--case', 'N4K2D2S4']
    runner.main()

raise SystemExit(supervise(run, 180, root / 'row-factor-acceptance-comparison.status.json'))
