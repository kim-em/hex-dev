...
record=dict(environment=sweep.environment(),cpu=cpu,stage='kernel-folds',inspector_sha256=hashlib.sha256(text.encode()).hexdigest())
observed=[]
try:
 ...
 out=Path('/tmp/issue-10320-profile/kernel-folds.json');out.write_text(json.dumps(record,indent=2))
 ...
 Path('/tmp/issue-10320-profile/kernel-folds.json').write_text(json.dumps(record,indent=2))
 raise
finally:
