...
Divide by constant coefficients normally. For symbolic denominators, expand
only when the numerator normalizes to at most one monomial. Otherwise preserve
the quotient as an atom and discard the speculative atom-table additions.
The existing inverse evaluator produces at most one monomial even for inverse
sums. No matrix structure is inspected. -/

open Lean Meta Qq Mathlib.Tactic Mathlib.Tactic.Ring Mathlib.Tactic.Ring.Common
namespace Determinant.Division
meta section
...
partial def eval  {u : Lean.Level}
  ...
  | ``HDiv.hDiv, _, some dsα | ``Div.div, _, some dsα => match e with
    | ~q($a / $b) => do
      ...
      let ⟨_, va, pa⟩ ← eval rc c a
      let ⟨_, vb, pb⟩ ← eval rc c b
      let single := match va with
        | .zero | .add _ .zero => true
        | _ => false
      let constant := match vb with
        | .zero | .add (.const _) .zero => true
        | _ => false
      unless single || constant do
        -- These trial normal forms are discarded. Do not leave their atoms
        -- in the table scanned by later normalization steps.
        metaSaved.restore
        set saved
        return ← els
