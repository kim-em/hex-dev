...
meta structure Cache (rα : Q(CommRing $α)) where
  entries : ExprMap (Cert rα) := {}
  repeated : ExprSet := {}

meta partial def countArithmetic (e : Expr) (counts : ExprMap Nat := {}) : ExprMap Nat := Id.run do
  let counts := counts.insert e (counts.getD e 0 + 1)
  if e.isAppOfArity ``HAdd.hAdd 6 || e.isAppOfArity ``HMul.hMul 6 then
    return countArithmetic e.getAppArgs[5]! (countArithmetic e.getAppArgs[4]! counts)
  if e.isAppOfArity ``Neg.neg 3 then return countArithmetic e.getAppArgs[2]! counts
  return counts

meta partial def countGoals (e : Expr) (counts : ExprMap Nat := {}) : ExprMap Nat :=
  if let some (a, b) := e.and? then countGoals b (countGoals a counts)
  else if let some (_, a, b) := e.eq? then countArithmetic b (countArithmetic a counts)
  else counts
...
meta partial def cached (e : Q($α)) :
    StateT (Cache rα) (CertM rα) (Cert rα) := do
  if let some c := (← get).entries.get? e then return c
...
meta partial def prove (g : MVarId) :
    StateT (Cache rα) (CertM rα) Unit := do
...
meta def runCached (useOpaque : Bool) : TacticM Unit := withMainContext do
  let g ← getMainGoal
  ...
  let ctx : Ctx rα := {
    ...
    arrayExpr := q(#[] : Array $α)
    arrayEntries := #[] }
  let counts := countGoals (← instantiateMVars (← g.getType))
  let repeated := if useOpaque then counts.fold (fun set e n => if n > 1 then set.insert e else set) ({} : ExprSet) else {}
  AtomM.run .reducible <| ((prove (rα := rα) g).run' { repeated }).run' {} |>.run ctx

elab "cached_ring" : tactic => runCached false
elab "sealed_ring" : tactic => runCached true
...
/-- With asynchronous elaboration disabled, `addDecl` performs its kernel
check before returning. This clock includes statement elaboration, tactic
work, all auxiliary declarations and the final kernel check without enabling
the profiler. Parsing and import loading precede this command. -/
elab "#measure_decl " cmd:command : command => do
  let start ← IO.monoNanosNow
  Lean.Elab.Command.withScope (fun s => { s with opts := (Elab.async.set s.opts false).setBool `profiler false }) do
