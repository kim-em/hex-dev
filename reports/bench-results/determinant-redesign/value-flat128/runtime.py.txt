...
def oracle(kind, entries, value, moduli=()):
    ...
    if kind == 'int':
        ...
        def poly(p):
            return ctx.from_dict({tuple(m): c for m, c in p})
        matrix = [[poly(p) for p in row] for row in entries]
        result = ctx.constant(0)
        for perm in itertools.permutations(range(4)):
            term = ctx.constant((-1) ** sum(perm[i] > perm[j]
...
def main():
    ...
    parser.add_argument('--ring', choices=['int', 'rat', 'dyadic', 'poly'], required=True)
    ...
    if args.ring == 'poly' and (args.dimension != 4 or args.shape != 'dense'):
    ...
    files = [p for p in files if p.name in
             ['runtime.py', 'Schedules.lean', 'Runtime.lean', 'Fixture.lean', 'Modular.lean']]
