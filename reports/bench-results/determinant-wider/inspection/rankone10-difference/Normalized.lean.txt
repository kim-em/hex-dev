...
meta def compareTarget {u : Level} {α : Q(Type u)} {rα : Q(CommRing $α)}
    (right : Q($α)) (compose direct : Bool) : CertM rα Expr := do
  let left ← certBirdDet
  if direct then
    if ← isDefEq left.norm right then return left.proof
  let target ← certEval right
  let ctx ← read
  unless left.val.eq rcℕ ctx.rc target.val do throwError "unequal normal forms"
...
meta def run (compose : Bool) (direct : Bool := false) : TacticM Unit := withMainContext do
  let g ← getMainGoal
  ...
  let proof ← compareTarget (rα := reified.rα) right compose direct |>.run' {} |>.run ctx |>.run .reducible
