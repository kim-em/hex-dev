...
The [residual and eight-curve construction record](bench-results/hex-primality-ecm-stage2-issue-10362.json)
, [initial 64-curve record](bench-results/hex-primality-ecm-prototype-paths-issue-10362.json),
...
provider first retains any core result already sufficient for the square-root
criterion. Otherwise it runs the unchanged p−1/rho portfolio first, then tries parameters
...
parameters and five bounds; empty intervals; stage-2 success; whole and
...
giant coordinates against fresh direct scalar ladders; and provider attempt
limits and exact factor/residual reconstruction. The fixed success cases are
...
Pollard p−1 stage 2 remains **unavailable** in the measured checkout.
Issue #10298 delivered a SPEC contract; `HexPrimality/PMinusOne.lean` still
implements only stage 1. The ECM experiment neither substitutes for its tests
nor assumes its eventual usefulness on these inputs.
