...
| HexNumberFieldTower | Resolved: recursive inversion passes its model after the extended-gcd chain was normalized (an implementation defect, values unchanged); division and dense `toPrimitive` are canonical mode-3 cases with recorded attempted schedules; negation has passing dense linear evidence; every advertised operation has an admissible mode and the report has no Concern. | [#9665](https://github.com/kim-em/hex-dev/issues/9665), [#9815](https://github.com/kim-em/hex-dev/issues/9815), [#9944](https://github.com/kim-em/hex-dev/issues/9944), [#9945](https://github.com/kim-em/hex-dev/issues/9945), [#9946](https://github.com/kim-em/hex-dev/issues/9946), [#9947](https://github.com/kim-em/hex-dev/issues/9947) |
...
covers only the integer-factorization phase (1.42% of the whole-thread
profile, or 2.44% of the resolved `factor?` frame) and uses a canonical
mode-3 case instead.
