...
All 306 candidate samples passed the preregistered 60-second absolute ceiling. The largest observed fresh-module time was 18.466 seconds. Comparator status is **no-comparable-surface-in-named-comparator**; no ratio or speedup is claimed.

Source commit: `abf57a6f24ade1e687c7d0ebb6d83ad59fc3dfef`. Host: `chungus2`. Each process used CPU 30 with `LEAN_NUM_THREADS=1`. The three companion sweeps ran concurrently on separately leased CPUs. Host activity and SMT sibling usage are retained as context; every completed sample counts.
...
|---|---|---:|---:|---:|---:|
| `chain-conjugate` | quotient | 8 | 4.031 | 4.074 | 1.260 |
| `empty-columns` | quotient | 1 | 2.631 | 2.681 | 0.004 |
| `empty-rows` | quotient | 1 | 2.633 | 2.740 | 0.004 |
| `entrywise-literal` | quotient | 1 | 18.337 | 18.466 | 2.680 |
| `large-coefficients` | quotient | 16 | 4.077 | 4.142 | 1.345 |
| `rank-deficient` | quotient | 8 | 3.781 | 3.846 | 1.030 |
| `rectangular-presentation` | quotient | 16 | 8.131 | 8.240 | 5.190 |

[All case medians and baseline deltas](bench-results/hex-smith-mathlib-tactic-probes-abf57a6f24ad-chungus2.md), [complete raw evidence](bench-results/hex-smith-mathlib-tactic-probes-abf57a6f24ad-chungus2.json.gz), and [certificate and artifact measurements](bench-results/hex-smith-mathlib-tactic-probes-abf57a6f24ad-chungus2-certificates.json) include all six samples, actual input and certificate bit heights, entry counts, serialized data sizes, emitted artifact sizes, axiom sets and source/toolchain/host provenance. Certificate byte counts measure the UTF-8 `reprStr` serialization of the literal data; emitted Lean artifact sizes are recorded separately.
