...
Current at revision `f396965d439aeaffcb3f843d85998b23765a22b2`, Hex measured
2026-08-22 on `chungus2` (AMD EPYC 9455, NixOS 26.11, Linux x86-64) on
verified-idle core 17 selected by `scripts/bench/idle_core.py`; the external
systems retain their 2026-08-01 same-protocol record on the same host. The
committed 392-row corpus has SHA-256
...
`reports/bench-results/hexbz-factor-sweep-f396965d-hex-chungus2-cpu17.json`
(SHA-256
`84d61aba782ee7e947f26b1610326575ac46425f2fb8050babe1ef280318dbc6`);
external record:
...
|---|---:|---:|---:|---:|---:|
| Hex public factorization | 383 | 9 | 272.314 us | 6.692 ms | 4.009 s |
...
overhead), Hex divided by verified Isabelle BZ has median `0.513x`,
p10-p90 `0.318x-1.542x`, and a 149-36 win split. The optimized unverified
libraries remain substantially faster: median Hex ratios are `6.706x`
against FLINT, `6.533x` against PARI, and `3.120x` against NTL on 80, 86,
and 139 eligible pairs.

| Family | Eligible pairs | Median Hex / Isabelle | Hex wins |
|---|---:|---:|---:|
| Chebyshev | 7 | 0.404x | 7 |
| Conway | 69 | 0.458x | 50 |
| Cyclotomic | 23 | 0.462x | 19 |
| Cyclotomic products | 17 | 0.546x | 15 |
| Laguerre | 11 | 0.604x | 11 |
| Legendre | 12 | 0.502x | 11 |
| Random products | 21 | 0.462x | 20 |
| Swinnerton-Dyer products | 7 | 0.679x | 5 |
| Swinnerton-Dyer | 6 | 0.632x | 6 |
...
`0.513x`, i.e. Hex ahead), and the lattice-backed tail strictly beats it:
...
cumulative Hex/Isabelle ratio over ranks 125-140 at `0.711x` (rank 133),
...
record: `sd4` 867 µs, `sd5` 7.007 ms, `sd6` 28.258 ms, `sd7` 202.355 ms,
