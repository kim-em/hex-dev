...
clean revision `0b8037d0` on verified-idle core 3 selected by the same script;
...
overhead), Hex divided by verified Isabelle BZ has median `0.499x`,
p10-p90 `0.315x-1.445x`, and a 161-36 win split. The optimized unverified
libraries remain substantially faster: median Hex ratios are `6.487x`
against FLINT, `6.302x` against PARI, and `2.968x` against NTL on 80, 86,
and 143 eligible pairs.

| Family | Eligible pairs | Median Hex / Isabelle | Hex wins |
|---|---:|---:|---:|
| Chebyshev | 8 | 0.422x | 8 |
| Conway | 75 | 0.448x | 56 |
| Cyclotomic | 23 | 0.457x | 19 |
| Cyclotomic products | 18 | 0.587x | 16 |
| Laguerre | 12 | 0.598x | 12 |
| Legendre | 13 | 0.488x | 12 |
| Random products | 23 | 0.450x | 22 |
| Swinnerton-Dyer products | 7 | 0.648x | 5 |
| Swinnerton-Dyer | 6 | 0.602x | 6 |
| Wilkinson | 12 | 1.092x | 5 |
...
`0.499x`, i.e. Hex ahead), and the lattice-backed tail strictly beats it:
