...
`lean-bench` registration or executable. The import block (including the
`import all` executable closure that lets the emitted certificate checks
and the kernel-fallback replays reduce) is identical across the baseline
and every measured probe, so the paired delta isolates elaboration and
kernel-checking of the emitted proof term.
-/
