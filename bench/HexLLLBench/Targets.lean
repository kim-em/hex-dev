...
/-- Steered-certification guard for the structured families (issue #8491). The
float-steered Gram-Schmidt never certifies on ajtai/q-ary/ntru/knapsack: on
every steered rung the candidate fails `lllReducedCheck (delta, 11/20)` and the
run falls back to `lllNative`. This records that state per family by running one
steered rung each (all at matrix dim >= 30, so `steerWins` routes to steering)
and pinning `certified = 0`, `fellBack = 4`. If steering is ever made to certify
on a structured basis, `certified` flips nonzero and this guard throws,
prompting an update to the comparator plots and HexLLL/PERFORMANCE.md, which
currently treat the structured steered curves as failed-attempt + exact-fallback
artifacts rather than successful steered reductions. -/
def runStructuredSteeredCertifyGuard : Unit -> IO Int := fun _ => do
  Hex.resetSteeredTally
  let targets : List (Unit -> IO Int) :=
    [runFirstShortVectorAjtaiNormSq32,
     runFirstShortVectorQaryNormSq32,
     runFirstShortVectorNtruNormSq16,
     runFirstShortVectorKnapsackNormSq32]
  for t in targets do
    discard <| t ()
  let tally <- Hex.steeredTally
  if tally.certified != 0 then
    throw <| IO.userError
      s!"issue #8491: steered now certifies on a structured family ({repr tally}); update the comparator plots and PERFORMANCE.md framing"
  return Int.ofNat tally.certified * 65537 + Int.ofNat tally.fellBack

setup_fixed_benchmark runStructuredSteeredCertifyGuard where {
    repeats := 1
    maxSecondsPerCall := 120.0
    expectedHash := some (Hashable.hash ((0 * 65537 + 4 : Int)))
  }
