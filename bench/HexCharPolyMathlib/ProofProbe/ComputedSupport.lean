...
namespace CharPolyComputedProbe
...
@[expose] def mulVec (w : List Int) : List (List Int) → List Int
  | [] => []
  | r :: rs => dot r w :: mulVec w rs

@[expose] def moments (B : List (List Int)) (r : List Int)
    (ts : List Int) (w : List Int) : Bool :=
  match ts with
  | [] => true
  | t :: rest => decide (Int.neg (dot r w) = t) &&
      match rest with
      | [] => true
      | _ :: _ => moments B r rest (mulVec w B)
...
@[expose] def steps : List (List Int) → List Step → List Int → Bool
  ...
  | (a :: r) :: rs, s :: ss, p =>
      let B := tails rs
      eqList p s.coefficients &&
      (match s.column with
        | one :: negA :: ts => decide (one = 1) && decide (Int.neg a = negA) &&
            Nat.beq ts.length rs.length && moments B r ts (heads rs)
        | _ => false) &&
...
elab "check_computed_kernel" : tactic => do
