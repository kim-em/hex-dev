...
/-! ## Stable hashes for prepared reflected inputs and certificates -/

instance : Hashable ZPoly where
  hash p := hash p.toArray

private def dyadicKey : Dyadic → Int
  | .zero => 0
...
private def formulaHash : Formula → UInt64
...
private def rootCmpTag : Separation.RootCmp → Nat
  | .lt => 0
  | .eq => 1
  | .gt => 2
...
private def certificateHash : Certificate → UInt64
  | .emptyIoc => hash (0 : Nat)
