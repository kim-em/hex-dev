...
This is also where the bundled `Lean.Grind` structure lives. The ring
instances hold for every modulus the type admits; the field laws do not, and
the reason is set out beside them.
-/
...
The laws above are the components; these instances package them so `grind` can
use them. `hex-gfq-field` carries the same bundle for the generic quotient
field, and until now the packed representation had none, so a goal that `grind`
closed over `FiniteField` had to be done by hand over `GF2nPoly`. -/
