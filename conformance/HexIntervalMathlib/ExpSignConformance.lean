...
private meta def emitSplit : MetaM Expr := do
  let left ← emitChild leftContext leftInput leftScope (mkConst ``leftSeed)
  let right ← emitChild rightContext rightInput rightScope (mkConst ``rightSeed)
  let result ←
