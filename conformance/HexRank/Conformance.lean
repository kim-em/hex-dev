...
-- the packed bounds: the same witnesses at slot width `64` and entry bound
-- `10`; `W` too small for `rank · modulus²`, and an entry bound below an
-- entry, are refuted
example : checkRankListPacked 64 10 3 4 (toLists kernelEx) kernelWitness = true := by decide +kernel
example : checkRankListPacked 64 10 3 4 (toLists kernelEx) kernelWitness9 = true := by decide +kernel
example : checkRankListPacked 8 10 3 4 (toLists kernelEx) kernelWitness9 = true := by decide +kernel
example : checkRankListPacked 8 10 3 4 (toLists kernelEx) kernelWitness = false := by decide +kernel
example : checkRankListPacked 64 5 3 4 (toLists kernelEx) kernelWitness = false := by decide +kernel
example : checkRankListPacked 64 10 3 4 (toLists kernelEx)
    { kernelWitness with vt := kernelWitness.vt.map (· ++ [0, 7]) } = true := by decide +kernel
...
#guard (rankWitness kernelEx3).toOption.map
    (fun w => checkRankListPacked 64 10 3 3 (toLists kernelEx3) w) = some true
