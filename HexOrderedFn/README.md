...
total refinement over the existing canonical `Hex.RationalFn` arithmetic.
It imports neither Mathlib nor an interval library. The full contract is in
...
a nonzero sign. Conditional bound division is separate from total field division.

`Real.enclose` uses an array Horner loop. Every coefficient and the new constant
receive the same requested width. `Real.attempt` signs a canonical fraction's
numerator and denominator bounds; formal zero returns immediately.
