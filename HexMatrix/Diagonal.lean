...
/-! Rectangular matrices supported on a leading diagonal. -/

namespace Hex.Matrix

universe u

/-- The `n × m` matrix carrying `d` down its leading diagonal and zero
everywhere else.  The diagonal vector may be shorter than either matrix
dimension; entries past either ambient dimension are simply not represented. -/
@[expose]
def diagMatrix {R : Type u} [OfNat R 0] {r : Nat}
    (d : Vector R r) (n m : Nat) : Matrix R n m :=
  Matrix.ofFn fun i j =>
    if h : i.val = j.val ∧ i.val < r then d[(⟨i.val, h.2⟩ : Fin r)] else 0

/-- Entry formula for `diagMatrix`. -/
@[grind =]
theorem getElem_diagMatrix {R : Type u} [OfNat R 0] {r n m : Nat}
...
@[simp]
theorem diagMatrix_apply_of_ne {R : Type u} [OfNat R 0] {r n m : Nat}
    (d : Vector R r) (i : Fin n) (j : Fin m) (hij : i.val ≠ j.val) :
    (diagMatrix d n m)[i][j] = 0 := by
  rw [getElem_diagMatrix]
  simp [hij]

/-- A represented diagonal entry is read from the source vector. -/
@[simp]
theorem diagMatrix_apply_diag {R : Type u} [OfNat R 0] {r n m : Nat}
    (d : Vector R r) (i : Fin n) (j : Fin m)
...
/-- A diagonal position beyond the source vector is zero. -/
@[simp]
theorem diagMatrix_apply_of_ge {R : Type u} [OfNat R 0] {r n m : Nat}
...
@[simp]
theorem row_diagMatrix_of_ge {R : Type u} [OfNat R 0] {r n m : Nat}
