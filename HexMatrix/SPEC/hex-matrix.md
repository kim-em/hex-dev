...
default-config `mulStrassen`, each annotated with its fitted power-law slope. The
naive series must sit near slope `3.0` and the Strassen series near
`log₂ 7 ≈ 2.81`, and the crossover where the two lines meet is marked. The two
visibly different slopes are the evidence the reader wants: they show Strassen
lowers the asymptotic order, not merely the constant factor. A speedup table
...
once at the end gives the same residue as reducing at each step. The equality is
provable from the existing `BarrettCtx.toNat_mulMod`.
