...
an explicitly chosen asymptotic window. The diagnostic target is a naive slope
near `3.0` and a Strassen slope near `log₂ 7 ≈ 2.81`, with the crossover where the
two lines meet marked. The fitted Strassen exponent is only a diagnostic, not an
acceptance condition: on the row-of-rows backing the quadrant-copy overhead and
the limited benched sizes bend the fit above `2.81`, and the exponent approaches
`log₂ 7` only once the sizes are large enough or the flat backing lands (see the
representation note below). The point the figure must make is the visibly
...
(128-bit, or reduced periodically before overflow) and reduces once at the end.
This is the standard delayed-reduction trick for modular matrix multiplication.
It is `Valid` because reduction modulo `p` is a ring homomorphism, so reducing
once at the end gives the same residue as reducing at each step. Proving that is
more than one lemma: the `Valid` proof needs an accumulator invariant (the
running sum modulo `p`), a no-overflow or periodic-reduction bound on the wide
accumulator, the final-reduction step, and the equality to the `ZMod64` / `Fp`
dot product the naive kernel computes. `BarrettCtx.toNat_mulMod` is the
single-multiply building block in that chain, not the whole proof.
