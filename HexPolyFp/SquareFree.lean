...
/-- Derivative-active completion wrapper with the state payload restricted to
the call path generated by the initial derivative split and Yun tail steps.

This is the consumable strategy for the derivative-active branch: callers
provide facts only for `yunFactorsDerivativeActiveReachable` states, avoiding
any universal provider for arbitrary raw Yun states. -/
private theorem yunFactorsLevelCompletes_of_derivative_active_stateProvider
    (hp : Hex.Nat.Prime p) (f : FpPoly p) (multiplicity fuel : Nat)
    (_hmultiplicity : 0 < multiplicity) (hfuel : f.size < fuel + 1)
    (hzero : f.isZero = false)
    (hdf : (DensePoly.derivative f).isZero = false)
